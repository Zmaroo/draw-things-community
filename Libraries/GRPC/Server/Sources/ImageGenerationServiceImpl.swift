  ...
  ImageGenerationService.SimpleServiceProtocol
{
  ...
  private let rawMaskCacheLock = DispatchQueue(label: "ImageGenerationServiceImpl.rawMaskCacheLock")
  private var rawMaskCache: [Data: Tensor<UInt8>] = [:]
  private var rawMaskCacheOrder: [Data] = []
  private let rawMaskCacheLimit = 8
  ...
  private func rawMaskCacheKey(
    data: Data, width: Int, height: Int, format: RawMaskFormat
  ) -> Data {
    var header = Data()
    var widthValue = UInt32(clamping: width)
    var heightValue = UInt32(clamping: height)
    var formatValue = UInt32(format.rawValue)
    withUnsafeBytes(of: &widthValue) { header.append(contentsOf: $0) }
    withUnsafeBytes(of: &heightValue) { header.append(contentsOf: $0) }
    withUnsafeBytes(of: &formatValue) { header.append(contentsOf: $0) }
    var combined = Data()
  ...
  private func cachedRawMaskTensor(
    for rawMask: RawMaskPayload,
    contents: [Data: Data]
  ) -> Tensor<UInt8>? {
    let width = Int(rawMask.width)
    let height = Int(rawMask.height)
    guard width > 0, height > 0 else {
      logger.warning("Raw mask ignored due to invalid size \(rawMask.width)x\(rawMask.height)")
      return nil
    }
    let resolvedData = unwrapContentData(rawMask.data, contents: contents)
    let expectedCount = width * height
    guard resolvedData.count == expectedCount else {
      logger.warning(
        "Raw mask ignored due to size mismatch bytes=\(resolvedData.count) expected=\(expectedCount)"
      )
      return nil
    }

    let key = rawMaskCacheKey(
      data: resolvedData,
      width: width,
      height: height,
      format: rawMask.format
    )
    if let cached = rawMaskCacheLock.sync(execute: { rawMaskCache[key] }) {
      return cached
    }

    let maskBytes: Data
    switch rawMask.format {
    case .u8Mask:
      maskBytes = resolvedData
    case .grayscale, .unspecified, .UNRECOGNIZED:
      var converted = Data(count: expectedCount)
      converted.withUnsafeMutableBytes { destBuffer in
        resolvedData.withUnsafeBytes { srcBuffer in
          guard
            let src = srcBuffer.baseAddress?.assumingMemoryBound(to: UInt8.self),
            let dst = destBuffer.baseAddress?.assumingMemoryBound(to: UInt8.self)
          else { return }
          for index in 0..<expectedCount {
            let value = src[index]
            let isProtected = value < 128
            dst[index] = isProtected ? 0xFB : 0xF9
          }
        }
      }
      maskBytes = converted
    ...
    var tensor = Tensor<UInt8>(.CPU, .NC(height, width))
    tensor.withUnsafeMutableBytes { destBuffer in
      guard let dst = destBuffer.baseAddress else { return }
      maskBytes.copyBytes(to: dst.assumingMemoryBound(to: UInt8.self), count: expectedCount)
    }

    rawMaskCacheLock.sync {
      rawMaskCache[key] = tensor
      rawMaskCacheOrder.append(key)
      if rawMaskCacheOrder.count > rawMaskCacheLimit, let removed = rawMaskCacheOrder.first {
        rawMaskCacheOrder.removeFirst()
        rawMaskCache.removeValue(forKey: removed)
      }
    }
    return tensor
  }

  private func unwrapContentData(_ data: Data, contents: [Data: Data]) -> Data {
    guard data.count == 32 else { return data }
    return contents[data] ?? Data()
  }

  private func resizedMask(
    _ mask: Tensor<UInt8>, width: Int, height: Int
  ) -> Tensor<UInt8> {
    let srcHeight = mask.shape[0]
    let srcWidth = mask.shape[1]
    guard srcHeight != height || srcWidth != width else {
      return mask
    }
    var resized = Tensor<UInt8>(.CPU, .NC(height, width))
    let scaleX = Double(srcWidth) / Double(width)
    let scaleY = Double(srcHeight) / Double(height)
    resized.withUnsafeMutableBytes { destBuffer in
      mask.withUnsafeBytes { srcBuffer in
        guard
          let src = srcBuffer.baseAddress?.assumingMemoryBound(to: UInt8.self),
          let dst = destBuffer.baseAddress?.assumingMemoryBound(to: UInt8.self)
        else { return }
  ...
  private func rawImageTensor(
    for rawImage: RawImagePayload,
    contents: [Data: Data]
  ) -> Tensor<FloatType>? {
    let width = Int(rawImage.width)
    let height = Int(rawImage.height)
    guard width > 0, height > 0 else {
      logger.warning("Raw image ignored due to invalid size \(rawImage.width)x\(rawImage.height)")
      return nil
    }
    let resolvedData = unwrapContentData(rawImage.data, contents: contents)
    let expectedCount = width * height * 4
    guard resolvedData.count == expectedCount else {
      logger.warning(
        "Raw image ignored due to size mismatch bytes=\(resolvedData.count) expected=\(expectedCount)"
      )
      return nil
    }
    guard rawImage.format == .rgba8 || rawImage.format == .unspecified else {
      logger.warning("Raw image ignored due to unsupported format \(rawImage.format)")
      return nil
    }
    ...
    tensor.withUnsafeMutableBytes { destBuffer in
      resolvedData.withUnsafeBytes { srcBuffer in
        guard
          let src = srcBuffer.baseAddress?.assumingMemoryBound(to: UInt8.self),
          let dst = destBuffer.baseAddress?.assumingMemoryBound(to: FloatType.self)
        else { return }
        let pixelCount = width * height
        for index in 0..<pixelCount {
          let base = index * 4
          let r = Float(src[base]) / 127.5 - 1.0
          let g = Float(src[base + 1]) / 127.5 - 1.0
          let b = Float(src[base + 2]) / 127.5 - 1.0
          let out = index * 3
          dst[out] = FloatType(r)
          dst[out + 1] = FloatType(g)
          dst[out + 2] = FloatType(b)
        }
      }
    }
    return tensor
  ...
  private func rewriteConfiguration(
    ...
    let cancellation = ProtectedValue<(() -> Void)?>(nil)
  ...
  private func generateImage(
    ...
    func cancel() {
      let didCancel = cancelFlag.exchange(true, ordering: .acquiringAndReleasing)
      guard !didCancel else { return }
      var cancellationBlock: (() -> Void)?
      cancellation.modify {
        cancellationBlock = $0
        $0 = nil
      }
  ...
  private func generateImage(
    ...
    func unwrapData(_ data: Data) -> Data {
      ...
      return unwrapContentData(data, contents: contents)
    }
    // Additional conversion if needed.
    var image: Tensor<FloatType>? = nil
    if request.hasRawImage {
      image = rawImageTensor(for: request.rawImage, contents: contents)
    } else if request.hasImage {
      image = Tensor<FloatType>(data: unwrapData(request.image), using: [.zip, .fpzip]).map {
        Tensor<FloatType>(from: $0)
      }
    }
    var mask: Tensor<UInt8>? = nil
    if request.hasRawMask {
      mask = cachedRawMaskTensor(for: request.rawMask, contents: contents)
    } else if request.hasMask {
      mask = Tensor<UInt8>(data: unwrapData(request.mask), using: [.zip, .fpzip])
    }
    if let mask, image == nil {
      let width = mask.shape[1]
      let height = mask.shape[0]
      var blank = Tensor<FloatType>(.CPU, .NHWC(1, height, width, 3))
      blank.withUnsafeMutableBytes { buffer in
        guard let ptr = buffer.baseAddress?.assumingMemoryBound(to: FloatType.self) else { return }
        ptr.initialize(repeating: 0, count: width * height * 3)
      }
      image = blank
    }
    if let image, let currentMask = mask {
      let imageWidth = image.shape[2]
      let imageHeight = image.shape[1]
      if currentMask.shape[1] != imageWidth || currentMask.shape[0] != imageHeight {
        logger.info(
          "Resizing raw mask from \(currentMask.shape[1])x\(currentMask.shape[0]) to \(imageWidth)x\(imageHeight)"
        )
        mask = resizedMask(currentMask, width: imageWidth, height: imageHeight)
      }
    }
